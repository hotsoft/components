...
type
  ...
  TacRadioComboPanel = class(tpanel)
  private
    FCaption: TCaption;
...
procedure Register;
begin
  RegisterComponents('Acras', [TacRadioComboPanel]);
end;

{ TacRadioComboPanel }

procedure TacRadioComboPanel.AdjustClientRect(var Rect: TRect);
begin
...
constructor TacRadioComboPanel.create(AOwner: TComponent);
begin
  inherited Create(AOwner);
end;

procedure TacRadioComboPanel.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TacRadioComboPanel.SetChecked(const Value: boolean);
var
  p: TComponent;
  i: integer;
begin
  ...
  begin
    ...
    begin
      if (p.components[i] is TacRadioComboPanel) then
        if p.Components[i] <> self then
          (p.components[i] as TacRadioComboPanel).Checked := false;
    end;
  end;
...
procedure TacRadioComboPanel.SetRadioCaption(const Value: TCaption);
begin
  FRadioCaption := Value;
  radioButton.Caption := Value;
end;

procedure TacRadioComboPanel.RadioButtonClick(Sender: TObject);
begin
  (Sender as TRadioButton).Checked := True;
  Checked := true;
end;

procedure TacRadioComboPanel.SetClosedHeight(const Value: integer);
begin
  FClosedHeight := Value;
end;

procedure TacRadioComboPanel.SetOpenedHeight(const Value: integer);
begin
  FOpenedHeight := Value;
end;

procedure TacRadioComboPanel.PanelTopResize(Sender: TObject);
begin
  if radioButton=nil then
    exit;
...
procedure TacRadioComboPanel.SetCaption(const Value: TCaption);
begin
  FCaption := Value;
end;


procedure TacRadioComboPanel.Resize;
begin
  inherited Resize;
{  if Checked then
  begin
