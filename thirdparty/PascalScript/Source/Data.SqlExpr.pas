...
var
  ...
  CreateLocalTransaction: Boolean;
begin
  Result := 0;
  if not FSQLConnection.InTransaction then
  begin
    CreateLocalTransaction := True;
    _Trans := FSQLConnection.BeginTransaction
  end
  else
  ...
  try
    try
      ...
      if (_Trans <> nil) and (CreateLocalTransaction) then
        FSQLConnection.CommitFreeAndNil(_Trans);
    end;
  except
    if (_Trans <> nil) and (CreateLocalTransaction) then
      FSQLConnection.RollbackFreeAndNil(_Trans);
  end;
end;
