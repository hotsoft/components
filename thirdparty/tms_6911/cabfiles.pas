...
function StrPasN(p: PAnsiChar): PAnsiChar;
begin
{$IFDEF DELPHIXE4_LVL}
  Result := PAnsiChar(AnsiStrings.StrPas(p));
{$ENDIF}
{$IFNDEF DELPHIXE4_LVL}
  Result := PAnsiChar(StrPas(p));
{$ENDIF}
end;
  ...
  fdintCOPY_FILE:
    begin
      ...
      begin
        ...
        begin
          Name := string(StrPasN(notif^.psz1));
      ...
      if (TCABFile(notif^.pv).FProcessMode = pmExtractAll) or
         ...
         (TCABFile(notif^.pv).CABFileContents.IsSelected(StrPasN(notif^.psz1)))) then
      begin
        {$IFNDEF DELPHI7_LVL}
        ...
        if  TCABFile(notif^.pv).ExtractRelative then
          TgtFile := string(StrPasN(notif^.psz1))
        else
          TgtFile := FileName;
        ...
        begin
          if Assigned(TCABFile(notif^.pv).FOnOverWriteFile) then
            TCABFile(notif^.pv).FOnOverWriteFile(TCABFile(notif^.pv),ExtractPath + string(StrPasN(notif^.psz1)),Allow);
...
begin
  ...
  {$ENDIF}
  DeleteFile(string(StrPasN(pszFile)));
  Result := 0;
  Err^ := 0;
end;
