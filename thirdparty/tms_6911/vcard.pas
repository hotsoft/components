...
begin
  ...
  try
    ...
    begin
      ...
      if Pos('BEGIN:VCARD', DelSpaces(s)) = 1 then
      begin
        vContact := vContacts.Add;
      end;

      if Pos('END:VCARD', DelSpaces(s)) = 1 then
      begin
        vContact := Nil;
        LastProp := '';
      end;

      if (Pos('VERSION', s) = 1) and Assigned(vContact) then
      begin
        if Pos('3.0', s) > 0 then
      ...
      if (Pos('UID:', s) = 1) and Assigned(vContact) then
      begin
        Delete(s, 1, 4);
        while Pos(':', s) > 0 do
        begin
      ...
      if (Pos('PRODID:', s) = 1) and Assigned(vContact) then
      begin
        Delete(s, 1, 7);
        vContact.ProdID := s;
      end;
      ...
      if (Pos('NICKNAME', s) = 1) and Assigned(vContact) then
      begin
        vContact.NickName := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('ORG', s) = 1) and Assigned(vContact) then
      begin
        vContact.Company := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('TITLE', s) = 1) and Assigned(vContact) then
      begin
        vContact.JobTitle := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('ROLE', s) = 1) and Assigned(vContact) then
      begin
        vContact.Profession := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('SOURCE', s) = 1) and Assigned(vContact) then
      begin
        vContact.Source := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('SORT-STRING', s) = 1) and Assigned(vContact) then
      begin
        vContact.SortString := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('TIMEZONE', s) = 1) and Assigned(vContact) then
      begin
        vContact.TimeZone := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('BDAY', s) = 1) and Assigned(vContact) then
      begin
        vContact.BirthDay := IsoToDate(GetPropVal(s));
        LastProp := '';
      end;

      if (Pos('CATEGORIES', s) = 1) and Assigned(vContact) then
      begin
        vContact.Categories.CommaText := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('GEO', s) = 1) and Assigned(vContact) then
      begin
        s := GetPropVal(s);
        i := 0;
      ...
      if (Pos('NOTE', s) = 1) and Assigned(vContact) then
      begin
        vContact.Note.Text := StringReplace(GetPropVal(s), '\n', #13, [rfReplaceAll]);
        LastProp := '';
      end;

      if (Pos('REV', s) = 1) and Assigned(vContact) then
      begin
        vContact.Updated := IsoToDateTime(GetPropVal(s));
        LastProp := '';
      end;

      if (Pos('EMAIL', s) = 1) and Assigned(vContact) then
      begin
      ...
      if (Pos('TEL', s) = 1) and Assigned(vContact) then
      begin
      ...
      if (Pos('ADR', s) = 1) and Assigned(vContact) then
      begin
        ...
        if (Pos('LABEL', props) > 0) and (vContact.vCardVersion = vv40) then
        begin
      ...
      if (Pos('X-SOCIALPROFILE',s) = 1) and Assigned(vContact) then
      begin
        si := vContact.SocialProfile.Add;
        props := GetProps(s);

        if pos('type=',props) > 0 then
        begin
          su := Copy(props,pos('type=',props)+5,length(props));
          if pos(';',su) > 0 then
        ...
        if pos('x-user=',props) > 0 then
        begin
          su := Copy(props,pos('x-user=',props)+7,length(props));
          if pos(';',su) > 0 then
      ...
      if (Pos('LABEL', s) = 1) and Assigned(vContact) and (adr <> nil) and
      ...
      if (Pos('URL', s) = 1) and Assigned(vContact) then
      begin
        vContact.WebsiteURL := GetPropVal(s);
        LastProp := '';
      end;

      if (Pos('PHOTO', s) = 1) and Assigned(vContact) then
      begin
        props := UpperCase(GetProps(s));
        if Pos('JPEG', props) > 0 then
          vContact.PhotoType := itJPEG
        else if Pos('GIF', props) > 0 then
          vContact.PhotoType := itGIF
        else if Pos('PNG', props) > 0 then
          vContact.PhotoType := itPNG;

        if (Pos('LUE=uri:', s) > 0) then
        begin
          vContact.PhotoEncoding := feExternalURL;
          vContact.PhotoURL := Copy(s, Pos('LUE=uri:', s) + 8, 65535);
          LastProp := 'PhotoURL';
        end
        else
        begin
...
begin
  ...
  if vContact.Company <> '' then
    Result := Result + 'ORG:' + vContact.Company + lf;
  if vContact.JobTitle <> '' then
    Result := Result + 'TITLE:' + vContact.JobTitle + lf;
  if vContact.Profession <> '' then
    Result := Result + 'ROLE:' + vContact.Profession + lf;
  if vContact.WebsiteURL <> '' then
    Result := Result + 'URL;WORK:' + vContact.WebsiteURL + lf;
  if vContact.Source <> '' then
    Result := Result + 'SOURCE:' + vContact.Source + lf;
  ...
  begin
    ...
    if vContact.PhotoEncoding = feInternalBase64 then//remarked by Ryan . i cloud donot support base64 encoding.
    begin
      case vContact.vCardVersion of
        // spec:
        ...
        vv40:
          begin
            Result := Result + 'PHOTO;ENCODING=b;TYPE=' + Pt + ':' + lf;
            //ls := Length('PHOTO;ENCODING=b;TYPE=' + Pt + ':');
          end;
      end;
