...
{$HPPEMIT ''}
{$HPPEMIT '#pragma link "shlwapi.lib"'}
{$HPPEMIT ''}
...
type
  ...
  procedure DrawButton(Canvas: TCanvas; AHandle: THandle; ARect: TRect; AControlStyle: TControlDrawStyle; AState: TControlState; ATones: TColorTones; ACaption: string = '');
...
begin
  ...
  CharToOem(pin,pout);
  {$IFDEF DELPHIXE4_LVL}
  s := string(AnsiStrings.StrPas(pout));
  {$ENDIF}
  {$IFNDEF DELPHIXE4_LVL}
...
begin
  ...
  {$IFDEF DELPHIXE4_LVL}
  AnsiStrings.StrPLCopy(pin,sa,Length(sa));
  {$ENDIF}
  {$IFNDEF DELPHIXE4_LVL}
...
begin
  ...
  begin
    ...
    DrawText(Canvas.Handle, PChar(ACaption), Length(ACaption), ARect, DT_CENTER);
  end;
