...
type
  ...
  public
    constructor Create;
    property Items[index: Integer]: Single read GetSingle write SetSingle; default;
    procedure Add(Value: Single);
    procedure Insert(Index: Integer;Value: Single);
  ...
  public
    ...
    property BezierLengths: TSingleList read FBezierLengths;
...
procedure TSingleList.SetSingle(Index: Integer; Value: Single);
begin
  inherited Items[Index] := Pointer(Value);
end;

function TSingleList.GetSingle(Index: Integer): Single;
begin
  Result := Single(inherited Items[Index]);
end;

procedure TSingleList.Add(Value: Single);
begin
...
var
  I: Integer;
begin
  FBezierLengths.Clear;
  for I := 0 to FBezierCount - 1 do
  begin
    CalculateSingleList(I * 4);
    FBezierLengths.Add(GetBezierLength);
  end;
end;
...
var
  ...
  pt: TGPPointF;
  pthx, pthy, tx, ty,
  px, py, mag, fx, txts, fy, bzl: Single;
  p: TGPPointF;
  bzp: TPointArray;
  bzt: TTypeArray;
  pthb: TGPRectF;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (I = pthlst.Count - 1) and (I <= FBezierLengths.Count - 1) then
...
begin
  Result := t;
  if FSingleList.Count > 0 then
  begin
    u := t;
    al := FSingleList.Count - 1;
    tal := u * FSingleList[al];
    idx := SearchVal(tal);
    if FSingleList[idx] = tal then
      Result := idx / al
    else if (idx >= 0) and (idx < FSingleList.Count - 1) then
    begin
      lbf := FSingleList[idx];
...
begin
  ...
  FSingleList := TSingleList.Create;
  FBezierLengths := TSingleList.Create;
end;
...
begin
  Result := 0;
  if FSingleList.Count > 0 then
    Result := FSingleList[FSingleList.Count - 1];
end;
...
begin
  Result := 0;
  for I := 0 to FSingleList.Count - 1 do
  begin
...
begin
  ...
  if FBezierLengths.Count = 0 then
    Exit;
  ...
  begin
    ...
    begin
      ...
      c := '';
      Inc(K);
      if K < FBezierLengths.Count - 1 then
        bzl := FBezierLengths[K]
      else
      begin
        J := Length(AText) - I;
